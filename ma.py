...
def place_mines(mine_board):
    ...
    mine_board = np.array([[False, False, False, False, True, True, False],
...
try:
    model.load_state_dict(torch.load("Baba1.pth"))
    print("Model loaded")
...
for episode in range(EPISODES):
    ...
    while not done:
        ...
        # Find the new action
        invalid_actions = np.nonzero(new_state[CLOSED].flatten() == False)[0]
        ...
        new_observation[invalid_actions] = -np.inf
        ...
        #         _, reward, done = step(temp_state, i)
    ...
    if episode % 1 == 0:
        ...
        print(f"Episode {episode}, Step {step_count}, score {score:4}, avg score {avg_score:4}, eps {EPSILON:.3f}, losses {np.mean(losses[-1000:]):.3f}")
...
torch.save(model.state_dict(), 'Baba1.pth')
# plot learning curve
